 ...
 * @author Wei Peng
 */
public class GetMysqlKeyBufferService {
    ...
    public ArrayList[] getKeyBuffer(InfoBean infoBean){
        ...
        for (int i = 0;i <array.length();i ++) {
            ...
            try {
                list1.add(Float.valueOf(jsonObject3.getString("key_buffer_read_hits"))*100);
            }catch (Exception e){
                list1.add((float) 0);
            }
            try {
                list2.add(Float.valueOf(jsonObject3.getString("key_buffer_write_hits"))*100);
            }catch (Exception e){
                list2.add((float) 0);
            }
        }
