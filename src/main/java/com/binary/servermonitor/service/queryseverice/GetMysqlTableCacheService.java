 ...
 * @author Wei Peng
 */
public class GetMysqlTableCacheService {
    ...
    public ArrayList[] getMyqlTableCache(InfoBean infoBean){
        ...
        for (int i = 0;i <array.length();i ++) {
            ...
            try {
                list1.add(Float.valueOf(jsonObject3.getString("Open_tables"))*100);
            }catch (Exception e){
                list1.add((float) 0);
            }
            try {
                list2.add(Float.valueOf(jsonObject3.getString("Opened_tables"))*100);
            }catch (Exception e){
                list2.add((float) 0);
            }
        }
