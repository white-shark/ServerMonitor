 ...
 * @author Wei Peng
 */
public class GetMysqlFrmCacheService {
    ...
    public ArrayList[] getMyqlFrmCache(InfoBean infoBean){
        ...
        for (int i = 0;i <array.length();i ++) {
            ...
            try {
                list1.add(Float.valueOf(jsonObject3.getString("Open_table_definitions")));
            }catch (Exception e){
                list1.add((float) 0);
            }
            try {
                list2.add(Float.valueOf(jsonObject3.getString("Opened_table_definitions")));
            }catch (Exception e){
                list2.add((float) 0);
            }
        }
