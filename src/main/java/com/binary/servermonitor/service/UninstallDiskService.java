...
import com.binary.servermonitor.data_api.disk.UninstallDiskApi;
 ...
 * @author Wei Peng
 */
public class UninstallDiskService {
    public String uninstallDisk(UserInfo userInfo,String volumeId,String hostId){

        UninstallDiskApi uninstallDiskApi = new UninstallDiskApi();
        Requestparameters requestparameters = new Requestparameters();
        requestparameters.setAction("DetachVolume");
        ...
        for (int i = 0;i < region.length;i ++){
            ...
            for (int j = 0;j < array.length();j ++){
                if (hostId.equals(array.getString(j))){
                    ...
                    String json = uninstallDiskApi.url(requestparameters,volumeId,hostId);
